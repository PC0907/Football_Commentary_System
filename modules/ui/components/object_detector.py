...
class ObjectDetector:
    ...
    def __init__(self, model_path=None, conf_threshold=0.5, device=None):
        self.model_path = Path(model_path).resolve() if model_path else Path(__file__).resolve().parent / "best_object.pt"
    ...
    def _load_model(self):
        if not self.model_path.exists():
            raise FileNotFoundError(f"Object detection model not found: {self.model_path}")
...
def process_video(video_path, model_path=None, output_dir='./output'):
    try:
